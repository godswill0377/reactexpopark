...
import DefaultSlide from './DefaultSlide.js';
...
opacity: 0.2,
borderRadius: 3,
backgroundColor: 'rgba(0, 85, 255, 0.20)'
  },
  skipLabel: 'Skip',
  doneLabel: 'Done',
  nextLabel: 'Next',
...
export default class AppSlider extends React.Component {
  ...
  _renderDefaultButton = name => {
    let buttonName = `${name.toLowerCase()}Label`;
    let content = (
    ...
    if (buttonName === 'nextLabel') {
      content = (
        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
        {content}
        <ArrowLogo width={24} height={24} />
          
        </View>
      );
    } 
    if (buttonName === 'doneLabel') {
      content = (
  ...
  render() {
    ...
    const activeDotStyle = defaultProps.activeDotStyle;
    console.log(activeDotStyle);
    return (
