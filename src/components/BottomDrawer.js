...
export default class BottomDrawer extends Component {
  ...
  constructor(props) {
    ...
    this.DOWN_POSITION = this._calculateDownPosition(this.UP_POSITION, this.DOWN_DISPLAY, this.props.offset)
  ...
  render() {
    return (
      <Animator
        ...
        upPosition={this.props.isStatic ? this.DOWN_POSITION : this.UP_POSITION}
  ...
  transitionTo(position) {
    ...
    if (this._Animator.current) {
      this._Animator.current._transitionTo(position, () => { console.log('transition done') });
    }
  }
  transitionToMid() {
    this.UP_POSITION = SCREEN_HEIGHT /2;
    this.DOWN_POSITION = SCREEN_HEIGHT /2;
    this.setState({
      upPosition: SCREEN_HEIGHT / 2,
      downPosition: SCREEN_HEIGHT/2
    })
    if (this._Animator.current) {
      this._Animator.current._transitionTo(this.UP_POSITION, () => { console.log('transition done') });
    }
  }
  goDownTransition(){
    if (this._Animator.current) {
      this._Animator.current._transitionTo(this.DOWN_POSITION, () => { console.log('down done') });
      this._resetUpPosition();
    }
  }
  ...
  goUpTransition(){
    if (this._Animator.current) {
      this._Animator.current._transitionTo(this.UP_POSITION, () => { console.log('down done') });
    }
  }
  _resetUpPosition(){
    this.UP_POSITION = this._calculateUpPosition(SCREEN_HEIGHT, this.props.containerHeight, this.props.offset)
  ...
  _calculateUpPosition(screenHeight, containerHeight, offset) {
    return {
      x: 0,
      y: screenHeight - (containerHeight -60 - offset) - 30
    }
  }

  _calculateDownPosition(upPosition, downDisplay, offset) {
    return {
      x: 0,
      y: upPosition.y + downDisplay - offset
    };
  }
}